...
class HybridRetrieverChain:
    ...
    def calculate_rrf_scores(self, input_dict):
        ...
        bm25_df = input_dict["bm25_results"].sort_values("score", ascending=False).reset_index(drop=True)
        faiss_df = input_dict["faiss_results"].sort_values("score", ascending=True).reset_index(drop=True)
